...
const ExamManagement = () => {
  ...
  const handleChangeStatus = async (exam, newStatus) => {
    try {
      ...
      showSuccessToast(
        `Đã chuyển trạng thái thành ${EXAM_STATUS[newStatus]?.label}`,
      );
      loadExams();
    } catch (err) {
    ...
    <div className="exam-management">
      <div className="page-header">
        <div className="header-left">
          <h1>
            <FileText size={24} /> Quản lý đề thi
          </h1>
          ...
          >
            ...
            {subjects.map((s) => (
              <option key={s.id} value={s.id}>
                {s.subjectCode} - {s.description || s.subjectName}
              </option>
            ))}
          </select>
          <select
              ...
              <tbody>
                {filteredExams.map((exam) => (
                  <tr key={exam.id}>
                    ...
                    <td>
                      {exam.subjectCode} - Khối {exam.gradeLevel}
                    </td>
        ...
        >
          ...
          >
            <div className="modal-header">
              <h2>
                <BarChart size={20} /> Thống kê đề thi
              </h2>
              <button
                className="close-btn"
                onClick={() => setShowStatisticsModal(false)}
              ...
              <div className="stat-grid">
                <div className="stat-card">
                  <div className="stat-value">
                    {examStatistics.totalQuestions}
                  </div>
                  <div className="stat-label">Tổng số câu</div>
                </div>
                <div className="stat-card">
                  <div className="stat-value">
                    {examStatistics.fromQuestionBank}
                  </div>
            ...
            <div className="modal-body">
              <div className="detail-grid">
                ...
                <div className="detail-item">
                  <label>Môn học:</label>
                  <span>
                    {selectedExam.subjectName} ({selectedExam.subjectCode})
                  </span>
                </div>
                <div className="detail-item">
                  <label>Khối:</label>
              ...
              {selectedExam.questions?.length > 0 && (
                <>
                  <h3>
                    Danh sách câu hỏi ({selectedExam.questions.length} câu)
                  </h3>
                    ...
                    {selectedExam.questions.map((q, idx) => (
                      <div key={q.id || idx} className="question-item">
                        <div className="q-header">
                          ...
                          <span
                            className={`source-badge ${q.sourceFlag?.toLowerCase()}`}
                          >
