  ...
  return (
    <div className="resource-management-page" data-tour="rm-page">
                  ...
                  <div className="chip-list">
                    {filteredSubjects.map((s) => {
                      ...
                      return (
                        ...
                        >
                          {s.subjectName || s.name || s.description || "Môn học"}
                        </button>
                      );
                    })}
                    {!loadingSubjects && filteredSubjects.length === 0 && (
                      <span className="muted">Không có môn học phù hợp cho khối đã chọn.</span>
                    )}
                  </div>
                </div>
              ) : (
                    ...
                    >
                      {filteredSubjects.map((s) => (
                        <option key={s.id} value={String(s.id)}>
                          {s.subjectName || s.name || s.description || ""}
                        </option>
                      ))}
                    </select>
                  </div>
                      ...
                      <div className="chip-list">
                        {chapters.map((c) => {
                          ...
                          return (
                            <button
                              ...
                              type="button"
                              className="choice-chip"
                              onClick={() => handleSelectChapter(cid)}
                            >
                        ...
                        {!loadingChapters && chapters.length === 0 && (
                          <span className="muted">
                            {selectedSubject
                              ? `Môn ${selectedSubject.subjectName || selectedSubject.name || selectedSubject.description} chưa có chương.`
                              : "Chưa có chương."}
                          </span>
                        )}
                      </div>
                        ...
                        <div className="compact-chip-block">
                          ...
                          <div className="chip-list">
                            ...
                            {!loadingLessons && lessons.length === 0 && (
                              <span className="muted">
                                {selectedChapter
                                  ? `Chương ${selectedChapter.chapterNumber} chưa có bài học.`
                                  : "Chưa có bài học."}
                              </span>
                            )}
                          </div>
      ...
      <div className="resources-section glass">
        <h3>
          <BookOpen size={18} /> Danh sách tài nguyên{" "}
          {lessonId && `(${resources.length})`}
        </h3>
        ...
        {!lessonId ? (
          ...
          <p className="muted">
            Chưa có tài nguyên nào. Hãy upload tài nguyên mới.
          </p>
        ) : (
                ...
                <div key={r.id} className="resource-card">
                  ...
                  <div className="resource-info">
                    ...
                    <span className="resource-date">
                      {new Date(r.createdAt || r.uploadedAt).toLocaleDateString(
                        "vi-VN",
                      )}
                    </span>
                  </div>
          ...
          >
            ...
            <div className="view-grid">
              ...
              <div className="view-item">
                ...
                <p>
                  {new Date(
                    viewingResource.createdAt || viewingResource.uploadedAt,
            ...
            {viewingResource.hasExtractedContent &&
              viewingResource.extractedContent && (
                <div className="view-item full">
