...
export default function ParentStudentDetail() {
    ...
    const visibleScores = scores.filter((x) => x.score != null && x.maxScore != null);
    const avgScore = visibleScores.length
        ? (visibleScores.reduce((s, x) => s + (x.score ?? 0), 0) / visibleScores.length).toFixed(1)
        : null;
    const maxScoreVal = visibleScores.length ? Math.max(...visibleScores.map(x => x.score ?? 0)) : null;
    const minScoreVal = visibleScores.length ? Math.min(...visibleScores.map(x => x.score ?? 0)) : null;
    const passedCount = visibleScores.filter(x => x.status === 'PASSED' || (x.maxScore && x.score / x.maxScore >= 0.5)).length;
                                            ...
                                            {scores.map((item) => (
                                                <tr key={item.resultId}>
                                                    ...
                                                    <td>
                                                        ...
                                                        >
                                                            {item.score == null ? 'Chờ công bố' : item.score}
                                                            {item.score != null && item.maxScore ? `/${item.maxScore}` : ''}
                        ...
                        {visibleScores.length === 0 ? (
                            ...
                            <>
                                <div className="psd-score-summary">
                                    ...
                                    <div className="psd-sum-item teal"><span className="psd-sum-val">{Math.round(passedCount / visibleScores.length * 100)}%</span><span className="psd-sum-lab">Tỷ lệ đạt</span></div>
                                </div>
                                <div className="psd-side-stat-row"><span>Tổng bài thi</span><strong>{visibleScores.length}</strong></div>
