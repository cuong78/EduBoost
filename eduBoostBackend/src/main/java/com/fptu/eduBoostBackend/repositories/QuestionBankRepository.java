...
AND (:chapterId IS NULL OR q.lesson.chapter.id = :chapterId)
...
JOIN FETCH q.cognitiveLevel c
JOIN FETCH q.createdBy u
WHERE (:lessonId IS NULL OR l.id = :lessonId)
AND (:cognitiveLevelId IS NULL OR c.id = :cognitiveLevelId)
AND (:sourceType IS NULL OR q.sourceType = :sourceType)
...
SELECT q FROM QuestionBank q
JOIN FETCH q.lesson
JOIN FETCH q.cognitiveLevel
JOIN FETCH q.createdBy
WHERE q.id = :id
""")
    Optional<QuestionBank> findDetailById(@Param("id") Long id);
